...
from flax.nnx import filterlib, graph, Object, Variable
...
class NDCG(Average):
    """
    Wrap Rax's nDCG computation for NNX.
    """
    ...
    def update(
        self, *, relevance: jax.Array, relevance_labels: jax.Array, mask: jax.Array, **_
    ):
        values = rax.ndcg_metric(
            scores=relevance,
...
class MRR(Average):
    """
    Wrap Rax's MRR computation for NNX.
    """
...
class NegativeLogLikelihood(Average):
    """
    Wrap Rax's NLL computation for NNX.
    """

    def update(
        self, *, click: jax.Array, click_labels: jax.Array, mask: jax.Array, **_
    ):
        values = rax.pointwise_sigmoid_loss(
            scores=click,
            labels=click_labels,
            where=mask,
            reduce_fn=reduce_per_query,
