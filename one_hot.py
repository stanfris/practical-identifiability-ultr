...
class TwoTowerModel(nnx.Module):
    def __init__(self, positions: int, query_doc_features: int, rngs: nnx.Rngs, bias_init_values: jnp.ndarray | None = None, frozen_param_idx: int | None = None, frozen_param_val: float | None = None):
    ...
    def __call__(self, batch):
        ...
        if self.frozen_param_idx is not None:
            # select bias where in batch the position is equal to idx, and set those to val
            bias = jnp.where(batch["positions"] == self.frozen_param_idx, self.frozen_param_val, bias)
        relevance = self.relevance(batch["query_doc_features"])
        return (bias + relevance).squeeze(-1)
    def loss_fn(self, batch, bias_mean_weight: float = 0.3):
        logits = self(batch)
        labels = batch["clicks"]

        task_loss = sigmoid_binary_cross_entropy(logits, labels).mean()

        position_bias = self.get_position_bias()
        bias_mean_loss = jnp.mean(position_bias) ** 2
        return task_loss + bias_mean_weight * bias_mean_loss
